...
public class SdpMessage {
	...
	public SdpMessage(SdpMessage sd) {
		...
		z=sd.z;
		k=sd.k;
		for (int i=0; i<sd.av.size(); i++) addAttribute(sd.av.elementAt(i));
		for (int i=0; i<sd.media.size(); i++) addMediaDescriptor(sd.media.elementAt(i));
	}
	...
	public SdpMessage(String sdp) {
		...
		if (tt.size()>0) tv = tt.toArray(new TimeDescription[0]);
