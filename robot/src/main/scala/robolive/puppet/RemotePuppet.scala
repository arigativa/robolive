...
class RemotePuppet(ip: String, port: Int) extends AutoCloseable {
  ...
  private val commandResponsePromises = new ConcurrentHashMap[String, (Instant, Promise[PuppetOutput])]()

  private val commandsObserver: StreamObserver[Puppet.Command] = puppetController.execute(new StreamObserver[PuppetOutput] {
