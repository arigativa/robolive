...
import ru.arigativa.robolive.wsrelay.exchange.{MessageExchange, WSMessageExchange}
...
object ServerApp extends IOApp {
  ...
  def makeHttpApp(messageExchange: MessageExchange[IO, String, String])(implicit L: Logger[IO]): HttpApp[IO] = {
    implicit val http4Dsl: Http4sDsl[IO] = Http4sDsl[IO]
    import http4Dsl._
    import org.http4s.implicits._

    HttpRoutes.of[IO] {
      case GET -> Root => WebSocketBuilder[IO].build1(
